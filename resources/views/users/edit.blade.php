...
@section('content_header')
    <h1 class="m-0 text-dark">管理者編集</h1>
@stop
                ...
                <div class="card-body">
                    <form action="{{ route('user.update', $user->id) }}" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" id="email" name="email" class="form-control" placeholder="aaa@example.com" value="{{ old('email', $user->email) }}">
                            @error ('email')<small class="text-muted">{{ $message }}</small>@enderror
                        </div>
                        <div class="form-group">
                            <label for="name">名前</label>
                            <input type="text" id="name" name="name" class="form-control" placeholder="山田太郎" value="{{ old('name', $user->name) }}">
